...
def build_rows(workbook_path: Path) -> list[dict]:
    ...
    # Ligne 4 (index) = en-tetes reelles (les 3 premieres lignes sont un
    # titre/date d'edition, pas des donnees). Colonnes : Designation,
    # Categorie, Stock actuel, Seuil_Alerte, Conso 3 mois, Conso 12 mois,
    # Cmd_BC, Cmd_import, DateLogistique, Observation.
    for row in sheet.iter_rows(min_row=5, values_only=True):
        ...
        rows_by_key[article_normalise] = {
            ...
            "categorie": str(row[1]).strip() if row[1] else None,
            "stock_actuel": to_number(row[2]),
            "seuil_alerte": to_number(row[3]),
            "conso_3_mois": to_number(row[4]),
            "conso_12_mois": to_number(row[5]),
            "cmd_bc": to_number(row[6]),
            "cmd_import": to_number(row[7]),
            "date_logistique": str(row[8]).strip() if row[8] else None,
            "observation": str(row[9]).strip() if row[9] else None,
        }
...
def post_chunk(url: str, api_key: str, rows: list[dict]) -> None:
    ...
    request = urllib.request.Request(
        f"{url}/rest/v1/stock_alertes_matiere_premiere?{query}",
...
def main() -> int:
    ...
    if not rows:
        print("No alert rows found in workbook.")
        return 1
    ...
    print(f"Import complete: {len(rows)} alert rows processed.")
    return 0
