-- Bug reel confirme (bloquait TOUTE sortie normale sur certains lots,
-- ex: "ancien-lot" sur les articles PRO WHITE) : la verification de stock
-- disponible comparait par code_normalise (souvent NULL ou incoherent sur
-- les vieux lots - vu en base : memes lignes "ancien-lot" avec
-- code_normalise NULL, "ANCIEN LOT" (espace) ou absent), alors que le stock
-- REELLEMENT AFFICHE comme disponible (dropdown Sortie MP, Inventaire MP)
-- vient de stock_mp_lot_balances() qui regroupe par numero_lot, jamais par
-- code_normalise. Resultat : la page montrait 38787 pcs disponibles pour
-- "ancien-lot" mais la RPC refusait la sortie avec "disponible : 0" - la
-- verification utilisait une cle differente de celle qui determine ce qui
-- est montre comme disponible. Repro confirmee avant ce correctif :
-- stock_mp_record_sortie_batch([{article_id:7947, numero_lot:"ancien-lot",
-- quantite:1, admin:false}]) -> "Stock insuffisant pour le lot ancien-lot
-- (disponible : 0, demande : 1)".
--
-- Redepose stock_mp_record_sortie_batch (remplace la version live de
-- fix_sortie_mp_dates_v2.sql, RIEN d'autre ne change) : la verification de
-- stock ET la recuperation de date_fabrication/date_expiration passent
-- toutes les deux de "code_normalise = v_code_normalise" a
-- "upper(trim(numero_lot)) = upper(trim(v_numero_lot))" - meme cle que
-- stock_mp_lot_balances(), pour que la RPC de sortie voit exactement le
-- meme stock que ce que l'ecran affiche comme disponible. L'insert
-- continue d'ecrire code_normalise normalement (aucun changement de
-- schema, juste la cle de lecture).
  ...
  loop
    ...
    if not v_admin then
      select coalesce(sum(qte_entree), 0) - coalesce(sum(qte_sortie), 0)
        ...
        where article_id = v_article_id and upper(trim(numero_lot)) = upper(trim(v_numero_lot));
    ...
    where article_id = v_article_id
      and upper(trim(numero_lot)) = upper(trim(v_numero_lot))
