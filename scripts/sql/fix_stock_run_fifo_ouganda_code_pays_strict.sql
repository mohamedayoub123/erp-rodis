...
--
-- Redepose stock_run_fifo (le "Dispatch" FIFO d'une commande) avec un seul
-- changement : pour un client Ouganda (_client_is_from_country(v_client,
-- 'ouganda'), meme detection que precedemment - base sur le pays du
-- client, pas son nom, donc s'applique a TOUS les clients Ouganda comme
-- Striker Limited), le stock SANS code pays n'est plus utilise du tout en
-- repli. Avant : un lot avec code pays etait seulement PRIORISE, mais le
-- Dispatch completait quand meme avec du stock normal si ca ne suffisait
-- pas. Demande explicite : "il prend SEULEMENT les articles qui ont le
-- code pays, si ya pas il faut dire que ya pas" - un client Ouganda ne
-- doit plus jamais recevoir de stock sans code pays ; si le stock avec
-- code pays ne couvre pas la quantite demandee, le reste remonte comme un
-- vrai manque (qt_non_dispo_total), exactement comme une rupture de stock
-- normale.
...
begin
  ...
  foreach v_ligne_id in array p_ordered_ligne_ids loop
    ...
    -- Client Ouganda : ne considere QUE les lots avec un code pays rempli -
    -- jamais de repli sur du stock sans code pays (demande explicite). Pour
    -- tout autre client, comportement inchange (tous les lots candidats).
    for v_lot in
      select lot_id, remaining, date_fabrication, chambre, code_pays
      from tmp_candidate_lots
      where not v_is_uganda_client or trim(coalesce(code_pays, '')) <> ''
      order by rule_rank
    loop
