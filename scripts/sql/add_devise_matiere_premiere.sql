-- Multi-devise pour les prix MP : FCFA (defaut) / USD / EUR, avec le taux de
-- change saisi a la main a chaque commande (fige sur la ligne, pas de taux
-- global).
...
begin
  ...
  insert into public.bons_commande_matiere_premiere (
    code, article_id, article_label, quantite, prix_unitaire, devise, taux_change, n_doss_4d, n_doss_erp, fournisseur, date_jour
  )
  select
    v_code,
    ...
    coalesce(nullif(elem->>'devise', ''), 'FCFA'),
    nullif(elem->>'taux_change', '')::numeric,
