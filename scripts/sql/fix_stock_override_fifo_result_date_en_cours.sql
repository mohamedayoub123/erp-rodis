...
-- (Vient apres fix_stock_override_fifo_result_livree.sql - remplace de
-- nouveau la meme fonction, sans danger a relancer.)
--
-- STATUT_DATE_EN_COURS / DATE_TRANSITION_STAND_ENCOURS (date "entree en
-- cours", lue par le rapport Statistique livraison pour calculer le delai
-- "hors stand") etaient reecrasees a la date du JOUR a CHAQUE clic sur
-- "Enregistrer tout" - meme sur une commande deja "en cours" depuis des
-- semaines. Desormais posee UNE SEULE FOIS (premiere fois que le token est
-- absent), jamais reecrasee ensuite. Meme correctif deja applique cote JS
-- (calculateFifoForCommandeAction/updateManualCommandeAction,
-- app/commandes/actions.ts).
...
begin
  ...
  -- Ne pose la date "entree en cours" que la premiere fois (token absent) -
  -- jamais reecrasee a chaque Enregistrer sur une commande deja en cours.
  if public._extract_comment_token(v_commentaire, 'STATUT_DATE_EN_COURS:') = '' then
