...
-- Ajoute sous_famille au resultat de stock_mp_display_rows (page Stock MP) -
-- demande explicite : meme colonne que categorie, deja affichee sur Articles
-- Matiere Premiere.
...
as $$
  ...
  depot_scoped as (
    select s.*, a.nom_article, a.gamme, a.categorie, a.sous_famille
    from split_rows s
  ...
  select
    p.display_key, p.id, p.article_id, p.nom_article, p.gamme, p.categorie, p.sous_famille, p.mouvement_type,
