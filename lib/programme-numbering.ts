...
// Retourne groupeId (PD) -> source_groupe_id (le PL d'origine, s'il existe
// encore) - l'inverse de fetchPdRefsBySourceGroupeId, pour la colonne "PL
// d'origine" sur Historique Programme Dispatcher.
export async function fetchSourceGroupeIdByPdGroupeId(): Promise<Map<number, number>> {
  const rows = await fetchAllPdGroupRows();

  const result = new Map<number, number>();
  for (const row of rows) {
    if (row.groupe_id === null || row.source_groupe_id === null) continue;
    if (!result.has(row.groupe_id)) {
      result.set(row.groupe_id, row.source_groupe_id);
    }
  }
