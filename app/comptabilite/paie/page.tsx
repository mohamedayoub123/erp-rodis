...
import { AddEmployeForm } from "./add-employe-form";
import { EmployeRow, type EmployeRowData } from "./employe-row";
...
async function fetchEmployesAvecPaiements(): Promise<EmployeRowData[]> {
  const { data: employesData } = await supabaseServer
    .from("employes")
    .select("id, nom, poste, salaire_mensuel, actif")
    .order("nom", { ascending: true });

  const employes = (employesData ?? []) as {
    id: number;
    nom: string;
    poste: string | null;
  ...
  return employes.map((employe) => {
    const paiements = [...ecritureIdBySourceId.entries()]
      .filter(([sourceId]) => sourceId.startsWith(`${employe.id}::`))
    ...
    return {
      ...
      nom: employe.nom,
      poste: employe.poste,
      salaireMensuel: Number(employe.salaire_mensuel),
      actif: employe.actif,
      paiements,
    };
  });
...
export default async function PaiePage() {
  ...
  const [comptes, employes] = await Promise.all([fetchAllComptes(), fetchEmployesAvecPaiements()]);
  const employesActifs = employes.filter((e) => e.actif);
  const employesInactifs = employes.filter((e) => !e.actif);
    ...
    <main className="min-h-screen bg-[linear-gradient(180deg,#f4efe5_0%,#fbf8f2_45%,#ffffff_100%)] px-4 py-6 text-slate-900 lg:px-8">
      <div className="mx-auto w-full space-y-6">
        <section className="rounded-[1.75rem] border border-black/5 bg-white p-5 shadow-[0_18px_40px_rgba(15,23,42,0.06)]">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h1 className="mt-2 text-3xl font-black tracking-tight text-slate-900">Paie</h1>
              <p className="mt-2 text-sm text-slate-600">
                Liste des employes et de leurs salaires - paye un mois pour generer l&apos;ecriture automatiquement.
              </p>
            </div>
        ...
        <section className="rounded-[1.75rem] border border-black/5 bg-white p-6 shadow-[0_18px_40px_rgba(15,23,42,0.06)]">
          <h2 className="mb-4 text-lg font-bold text-slate-900">Employes</h2>
          {employesActifs.length === 0 && employesInactifs.length === 0 ? (
            <p className="text-sm text-slate-500">Aucun employe pour le moment.</p>
          ) : (
            <div className="space-y-3">
              {[...employesActifs, ...employesInactifs].map((employe) => (
                <EmployeRow key={employe.id} employe={employe} canWrite={canWrite} />
              ))}
            </div>
          )}
        </section>
        ...
        {canWrite ? (
          <section className="rounded-[1.75rem] border border-black/5 bg-white p-6 shadow-[0_18px_40px_rgba(15,23,42,0.06)]">
            <h2 className="mb-4 text-lg font-bold text-slate-900">Ajouter un employe</h2>
            <AddEmployeForm comptes={comptes} />
          </section>
        ) : null}
      </div>
    </main>
