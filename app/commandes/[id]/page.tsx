...
import { fetchStockGroupsByArticle, fetchReservedByKey, buildAvailableCodesByArticle } from "./stock-codes";
...
export default async function CommandeDetailPage({
  ...
  // UNIQUEMENT les articles reellement presents dans cette commande (lignes
  // + resultats FIFO deja dispatches) - PAS tous les articles du systeme.
  // Avant, ce tableau incluait articleOptions.map(...) (TOUS les articles,
  // pour pre-calculer d'avance les codes disponibles du picker "Ajouter une
  // ligne", une fonctionnalite repliee par defaut et rarement ouverte) - ce
  // qui forcait fetchStockGroupsByArticle/fetchReservedByKey a lire le stock
  // de la totalite des articles a CHAQUE ouverture de commande, quelle que
  // soit sa taille (mesure : ~5s de cout fixe, identique sur une commande a
  // 24 comme a 97 lignes). Le picker recupere maintenant ses codes a la
  // demande (voir fetchAvailableCodesForNewArticleAction).
  const relevantArticleIds = [
    ...new Set([...fifoResults.map((ligne) => ligne.article_id), ...selectedArticleIds]),
  ];
  ...
  const [
    ...
    // Stock lu UNE SEULE FOIS pour les articles DEJA presents dans cette
    // commande (modifier une ligne existante) - le picker "Ajouter une
    // ligne" (produit hors commande) recupere ses propres codes a la
    // demande, voir fetchAvailableCodesForNewArticleAction.
