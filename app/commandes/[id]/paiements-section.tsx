...
import { deletePaiementCommandeAction, enregistrerPaiementCommandeAction } from "../actions";
...
// Paiements clients sur une commande - seul moyen de faire redescendre le
// compte 411000 (Clients) apres la facturation (voir enregistrerPaiementCommandeAction
// dans ../actions.ts). enregistrerPaiementCommandeAction leve une Error simple
// et ne renvoie rien (void) en cas de succes - meme idiome que FifoAddLigneForm :
// FormData construite a la main + useTransition + try/catch, pas de <form
// action={...}> natif ici (le formulaire d'ajout doit rester rempli pour
// afficher l'erreur/le succes et vider ses champs seulement au succes).
export function PaiementsSection({
  ...
  const restant = montantFacture !== null ? montantFacture - totalPaye : null;
  ...
  function handleSubmit() {
    ...
    startTransition(async () => {
      try {
        await enregistrerPaiementCommandeAction(formData);
      } catch (err) {
    ...
    <div className="mt-4">
      <div className="rounded-2xl bg-slate-50 px-4 py-3 text-sm font-medium text-slate-700">
        {montantFacture === null ? (
          ...
          <>
            Montant facture : {montantFacture.toLocaleString("fr-FR", { maximumFractionDigits: 0 })} FCFA
            {" — "}
            Paye : {totalPaye.toLocaleString("fr-FR", { maximumFractionDigits: 0 })} FCFA
            {" — "}
            Reste :{" "}
            <span className={restant !== null && restant > 0 ? "font-semibold text-red-600" : "font-semibold text-emerald-700"}>
              {(restant ?? 0).toLocaleString("fr-FR", { maximumFractionDigits: 0 })} FCFA
            </span>
          </>
        )}
      </div>
      ...
      {paiements.length > 0 ? (
                ...
                <tr key={paiement.id} className="border-t border-slate-200">
                  ...
                  {canEdit ? (
                    <td className="px-4 py-2">
                      <form action={deletePaiementCommandeAction}>
        ...
        <p className="mt-4 text-sm text-slate-500">Aucun paiement enregistre pour cette commande.</p>
      )}
