...
export function QuantitePourcentField({
  ...
  // Text (pas number) pour le %: un champ number rejette tout le contenu
  // colle des qu'il contient un caractere non numerique (ex: coller "10%"
  // au lieu de "10" videtoutel le champ sans rien afficher) - on nettoie
  // nous-memes (symbole %, virgule decimale) au lieu de laisser le
  // navigateur bloquer le coller silencieusement.
  function handlePourcent(raw: string) {
    const nettoye = raw.replace("%", "").replace(",", ".");
    setPourcent(nettoye);
    const p = Number(nettoye);
    setQuantite(total > 0 && nettoye && !Number.isNaN(p) ? String(round((p / 100) * total, 3)) : "");
  }

  return (
    <div className="flex items-center gap-2">
      <div className="grid gap-1">
        <span className="text-[0.65rem] font-semibold uppercase tracking-wide text-slate-400">QT</span>
        <input
          type="number"
          step="0.001"
          min="0"
      ...
      <div className="grid gap-1">
        <span className="text-[0.65rem] font-semibold uppercase tracking-wide text-slate-400">%</span>
        <input
          type="text"
          inputMode="decimal"
          value={pourcent}
