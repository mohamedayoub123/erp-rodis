...
import { unmarkVracTermineAction } from "../../actions";
...
async function fetchAllVracTermineRows(): Promise<CodeTermineRow[]> {
...
// Historique de tout ce qui a ete "Fin programme" en Fabrication - meme
// page que /production/suivi/dashboard/historique-fin-programme
// (Conditionnement), dupliquee ici pour la colonne Fabrication (stage
// "vrac"), demande explicite de l'utilisateur.
//
// Reservee aux comptes admin, meme logique que l'historique Conditionnement
// voisin.
export default async function HistoriqueFinProgrammeFabricationPage({
  searchParams,
}: {
  ...
  const [termineRows, pdLabelByCode] = await Promise.all([fetchAllVracTermineRows(), buildPdLabelByCode()]);
        ...
        <section className="overflow-hidden rounded-[1.75rem] border border-black/5 bg-white shadow-[0_18px_50px_rgba(15,23,42,0.08)]">
          {rows.length === 0 ? (
            <div className="px-6 py-8 text-sm text-slate-500">
              Aucun code Fabrication termine pour le moment.
            </div>
          ) : (
                  ...
                  {rows.map((row) => (
                    <tr key={row.id} className="border-t border-slate-100">
                      ...
                      <td className="px-6 py-4">
                        <form action={unmarkVracTermineAction}>
